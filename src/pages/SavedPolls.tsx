import React from 'react';
import { useSavedPolls } from '@/hooks/use-saved-polls';
import { usePollsByIds } from '@/hooks/use-polls-by-ids';
import { Card, CardContent } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import PollResultSummaryCard from '@/components/PollResultSummaryCard';
import { Bookmark } from 'lucide-react';
import { Link } from 'react-router-dom';

const SavedPollsPage: React.FC = () => {
  const { savedPolls, isLoading: isLoadingIds } = useSavedPolls();
  const pollIds = React.useMemo(() => Array.from(savedPolls), [savedPolls]);
  const { data: polls, isLoading: isLoadingPolls } = usePollsByIds(pollIds);

  const isLoading = isLoadingIds || isLoadingPolls;

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center space-x-3">
        <Bookmark className="h-8 w-8 text-primary" />
        <div>
          <h1 className="text-3xl font-bold">Saved Polls</h1>
          <p className="text-muted-foreground">
            Here are the polls you've saved for later.
          </p>
        </div>
      </div>

      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[...Array(3)].map((_, i) => (
            <Card key={i}><CardContent className="p-6"><Skeleton className="h-32 w-full" /></CardContent></Card>
          ))}
        </div>
      ) : !polls || polls.length === 0 ? (
        <Card>
          <CardContent className="p-10 text-center">
            <p className="text-lg text-muted-foreground">You have no saved polls.</p>
            <p className="text-sm text-muted-foreground mt-2">
              Click the <Bookmark className="inline h-4 w-4" /> icon on any poll on the{' '}
              <Link to="/poll-results" className="text-primary underline">Results</Link> page to save it here.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {polls.map(poll => (
            <PollResultSummaryCard key={poll.id} poll={poll} />
          ))}
        </div>
      )}
    </div>
  );
};

export default SavedPollsPage;